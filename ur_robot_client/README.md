...
| 예제               | 실행                                        | 내용                                         |
| ------------------ | ------------------------------------------- | -------------------------------------------- |
| `example_movej`    | `ros2 run ur_robot_client example_movej`    | HOME → TARGET → HOME 관절 공간 모션          |
| `example_io_speed` | `ros2 run ur_robot_client example_io_speed` | Speed slider 50/100% + DI 읽기 + DO 토글     |
| `example_complete` | `ros2 run ur_robot_client example_complete` | Pick & Place 통합 시퀀스 (모든 기능 사용)    |
| `example_state`    | `ros2 run ur_robot_client example_state`    | 읽기 전용 상태 모니터링 (실로봇 연결 점검용) |
