...
Universal Robots ROS2는 UR 협동로봇(ur3 ~ ur30)을 ROS2 환경에서 손쉽게 제어하기 위한 모션 제어 스택입니다. 공식 [UR ROS2 Driver](https://github.com/UniversalRobots/Universal_Robots_ROS2_Driver)와 MoveIt2를 래핑하고, 그 위에 단순한 Action 인터페이스(`/move_j`, `/move_l`)와 클라이언트 라이브러리(`URRobotClient`)를 얹어, 복잡한 MoveIt API 호출 없이 몇 줄의 코드로 관절/직교 공간 모션, 속도, 디지털 I/O를 제어할 수 있습니다.

상위 시스템(OPC UA 서버, MES, 비전 파이프라인 등)이 로봇 세부 구현을 몰라도 되도록 클라이언트 계층에서 추상화하며, 비상정지·Local 모드 전환으로 제어권을 잃어도 물리 복구가 끝나면 자동으로 제어권을 회복하는 program watchdog을 내장하고 있습니다.

### 주요 구성요소

- **ur_motion** (C++): MoveJ/MoveL Action Server. MoveIt2 백엔드로 충돌 회피 플래닝 후 궤적 실행
- **ur_robot_client** (C++): `URRobotClient` 클라이언트 라이브러리 + 예제 4종. 모션/속도/I/O/상태 조회 + program watchdog
- **ur_robot_client_py** (Python): 동일 기능의 Python 클라이언트 라이브러리 (non-Node 클래스, Node 주입 방식)
- **ur_robot_driver_wrapper**: UR ROS2 Driver 브링업 래퍼 (드라이버 + 컨트롤러)
...
- **MoveJ / MoveL Action**: 관절 공간(MoveJ) 및 직교 공간 직선(MoveL) 모션을 ROS2 Action으로 제공, MoveIt2 플래닝·충돌 검사 백엔드 사용
- **C++ / Python 클라이언트**: 동일한 기능의 `URRobotClient` 클래스를 양쪽 언어로 제공, Action/Service 호출을 `moveJ()`, `setSpeedSlider()` 같은 메서드로 추상화
- **Program Watchdog (자동 제어권 회복)**: e-stop·Local 모드로 external control 프로그램이 중단되면, 로봇 복구 시점에 `resend_robot_program`을 자동 호출하여 스택 재시작 없이 제어권 회복
- **Speed Slider 제어**: 속도 슬라이더 설정 + 실제 speed scaling 모니터링 (동기/비동기 모드)
...
- **Docker 지원**: ROS2 Humble + UR Driver + MoveIt2 포함 이미지, 빌드/실행 스크립트 제공
...
[워치독 흐름] program_running=false 감지 → 로봇 복구(RUNNING+NORMAL) 대기 → resend_robot_program → 제어권 회복
```
...
| 패키지                       | 출처 (버전)                                                                   | 용도                    |
| ---------------------------- | ----------------------------------------------------------------------------- | ----------------------- |
| Universal_Robots_ROS2_Driver | https://github.com/UniversalRobots/Universal_Robots_ROS2_Driver (humble, apt) | UR 공식 ROS2 드라이버   |
| MoveIt2                      | https://moveit.picknik.ai/humble (apt)                                        | 모션 플래닝 / 충돌 검사 |
...
Docker 이미지에 모든 의존성(UR Driver, MoveIt2, ros2_control 등)이 포함되어 있어 한 번의 빌드로 환경 준비가 끝납니다.
...
[run.sh](docker/run.sh)는 다음을 자동으로 처리합니다:
...
UR Driver + MoveIt + Motion Action Server를 한 번에 실행합니다.
...
> `run.sh`는 이미지 확인 · X11 권한 · 마운트 · 파일 소유권 복원(`HOST_UID`/`HOST_GID`) · 기존 컨테이너 재사용을 한 번에 처리합니다.
...
```
시스템 실행 ───▶ (실로봇) 캘리브레이션 ───▶ 클라이언트 연결 ───▶ 모션 / I/O 제어
     │                 │                       │                    │
 all.launch.py   extract_robot_        waitRobotReady()       moveJ / moveL
                 calibration.sh        is_connected()         setDigitalOut
                 (최초 1회)                                    setSpeedSlider
```

### 1. (실제 로봇, 최초 1회) 캘리브레이션 추출

정확한 TCP 위치를 위해 로봇 고유의 kinematics 캘리브레이션을 추출합니다.

```bash
# 스크립트 내부의 ROBOT_IP를 실제 로봇 IP로 수정 후 실행
...
`URRobotClient`(C++)는 500ms 주기 watchdog으로 external control 프로그램 상태를 감시합니다.

- e-stop/보호정지/Local 모드 전환 등으로 프로그램이 중단되면 경고 로그 출력
- 작업자가 물리적 복구(비상정지 해제, Remote 모드 전환)를 완료하면 — 즉 robot mode `RUNNING` + safety mode `NORMAL` — 자동으로 `resend_robot_program`을 호출하여 제어권 회복 (3초 간격 재시도)
...
- `isProgramRunning()`으로 현재 제어권 상태 조회 가능

---

## 설정

### 컨트롤러 설정 (`ur_robot_driver_wrapper/config/ur_controllers.yaml`)

```yaml
# 기본 사용 컨트롤러: scaled_joint_trajectory_controller
# io_and_status_controller: I/O, speed slider, 프로그램 상태 제공
```

### Kinematics 캘리브레이션 (`ur_description_wrapper/config/calibration_kinematics.yaml`)
...
[config.sh.example](docker/config.sh.example)을 복사해 사용하며, git에는 추적되지 않습니다.

```bash
IMAGE_NAME="universal-robots-ros2:latest"
CONTAINER_NAME="universal-robots-ros2"   # .vscode/clangd-docker.sh도 이 값을 읽음
...
[common.launch.py](common.launch.py)에서 정의하며 모든 launch 파일이 공유합니다.
...
| `/dashboard_client/is_in_remote_control`          | Service      | Remote/Local 모드 확인                         |
...
해결: e-stop이 눌리거나 Pendant가 Remote → Local로 전환되면 external control 프로그램이 멈추고 드라이버가 제어권을 잃습니다. 작업자가 물리적 복구(비상정지 해제, 전원/브레이크 해제, Remote 전환)를 마치면 `URRobotClient`(C++) 내장 watchdog이 `resend_robot_program`을 호출해 자동으로 제어권을 재획득합니다 — 스택 재시작이 필요 없습니다.
